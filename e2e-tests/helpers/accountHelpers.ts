...
export async function ensureSession(
	testAccountFirstName = generateTimeStamp(),
	testAccountLastName = "test",
) {
	...
	try {
		const successfulEmail = await createAccount(
			page,
			zipCode,
			role,
			password,
		...
		console.log(
			`Saving new test account: ${testAccountFirstName} ${testAccountLastName} - ${successfulEmail} - ${password + "1"}`,
		);
...
export async function ensureAdminSession(
	...
	if (fs.existsSync(ADMIN_SESSION_FILE)) {
		console.log(
			"Existing admin session found, deleting and creating a new one...",
		);
	...
	try {
		...
		console.log(
			`Marking ${testAccountFirstName} ${testAccountLastName} test account as verified...`,
		);
...
export async function loginAccount(page, emailAddress, password) {
	...
	for (let attempt = 1; attempt <= maxRetries; attempt++) {
		try {
			...
			try {
				...
				if (
					navError.message.includes("ERR_ABORTED") ||
			...
			if (!currentUrl.includes("/dashboard")) {
				throw new Error(
					`Login failed - not redirected to dashboard. Current URL: ${currentUrl}`,
				);
			}
			...
			if (attempt === maxRetries) {
				throw new Error(
					`Login failed after ${maxRetries} attempts: ${error.message}`,
				);
			}
...
export async function validateLoginPage(page, maxRetries = 2) {
	for (let attempt = 1; attempt <= maxRetries; attempt++) {
		try {
			...
			console.log(
				`Login page validation attempt ${attempt} failed:`,
				error.message,
			);
			if (attempt === maxRetries) {
				return false;
...
export async function createAccount(
	...
	for (let attempt = 1; attempt <= maxRetries; attempt++) {
		try {
			...
			const attemptEmailAddress =
				attempt === 1 ? emailAddress : generateEmail();
			...
			await page
				...
				.fill(attemptEmailAddress);
			await page
				.getByRole("textbox", { name: "phone" })
				.fill(attemptPhoneNumber);
			...
			await page
				.getByRole("button", { name: role, timeout: 60000 })
				.first()
				.click();
			...
			console.error(
				`Account creation attempt ${attempt} failed:`,
				error.message,
			);

			if (attempt === maxRetries) {
				throw new Error(
					`Account creation failed after ${maxRetries} attempts: ${error.message}`,
				);
			}
...
export async function upgradeToPro(page) {
	...
	await documentReady(page);
	await page
		.getByRole("heading", { name: "Please confirm your office details." })
		.isVisible();
	...
	await page
		.getByTestId("product-summary-product-image", { timeout: 10000 })
		.isVisible();
	...
	await page
		.getByRole("heading", {
			name: "You are now subscribed to GoodParty.org Pro!",
			timeout: 60000,
		})
		.isVisible();
...
export async function deleteAccount(page: Page | null = null) {
	const baseURL = process.env.BASE_URL;
	const SESSION_FILE = path.resolve(__dirname, "../auth.json");
	...
	let deleteButton: Locator, proceedButton: Locator;
	let lastError: any;
	...
	if (!deleteButton || !proceedButton) {
		throw (
			lastError ||
			new Error(
				"Failed to find and click Delete Account/Proceed button after retries",
			)
		);
	}
...
export async function useTestAccountCredentials(page) {
	...
	if (!fs.existsSync(testAccountPath)) {
		throw new Error(
			"Test account file not found. Run ensureSession() first to create a test account.",
		);
	}
	...
	try {
		...
		console.log(
			`Found test account credentials for: ${accountData.emailAddress}`,
		);
...
export async function useAdminCredentials(page) {
	...
	if (!adminEmail || !adminPassword) {
		throw new Error(
			"Admin credentials not found in environment variables. Please set TEST_USER_ADMIN and TEST_USER_ADMIN_PASSWORD.",
		);
	}
...
export async function createStableBrowserContext(
	browser: Browser,
	options = {},
) {
	...
	for (let attempt = 1; attempt <= maxRetries; attempt++) {
		try {
			...
			if (currentUrl.includes("/sign-up") || currentUrl.includes("/login")) {
				console.log(
					"Detected signup page - session is invalid, need manual login",
				);
				return false;
			}
			...
			if (isLoggedIn) {
				...
				console.log(
					`Session validation attempt ${attempt} failed - not logged in`,
				);
				if (attempt < maxRetries) {
					await page.waitForTimeout(1000);
				}
			...
			console.log(
				`Session validation attempt ${attempt} failed:`,
				error.message,
			);
			if (attempt < maxRetries) {
				await page.waitForTimeout(1000);
...
export async function prepareTest(type, url, text, page, browser = null) {
	...
	for (let attempt = 1; attempt <= maxRetries; attempt++) {
		try {
			...
			if (type === "admin") {
				if (
					fs.existsSync(adminSessionFile) &&
					(await isSessionRecent(adminSessionFile, 15))
				) {
					try {
						...
						console.log(
							"Recent admin session failed, trying with validation...",
						);
					}
				}
				...
				if (fs.existsSync(adminSessionFile)) {
					try {
						...
						if (isValidSession) {
							...
							console.log(
								"Admin session appears to be invalid, falling back to manual login...",
							);
						}
					} catch (error) {
						console.log(
							"Admin session login failed, falling back to manual login...",
							error.message,
						);
					}
				}
				...
				try {
					...
					console.log(
						"Warning: Could not save admin session:",
						saveError.message,
					);
				}
			}
			...
			if (type === "user") {
				if (
					...
					(await isSessionRecent(sessionFile, 30))
				) {
					try {
						...
						console.log(
							"Recent user session failed, trying with validation...",
						);
					}
				}
				...
				if (fs.existsSync(sessionFile)) {
					try {
						...
						if (isValidSession) {
							...
							console.log(
								"Session appears to be invalid, falling back to manual login...",
							);
						}
					} catch (error) {
						console.log(
							"Session login failed, falling back to manual login...",
							error.message,
						);
					}
				}
				...
				try {
					...
					console.log(
						"Warning: Could not save user session:",
						saveError.message,
					);
				}
			}
			...
			if (attempt === maxRetries) {
				throw new Error(
					`Authentication failed after ${maxRetries} attempts: ${error.message}`,
				);
			}
...
export async function authenticateWithTimeout(page, url, expectedText) {
	...
	console.log(
		`Starting authentication process (timeout: ${maxAuthTime / 1000}s)...`,
	);
		...
		await Promise.race([
			...
			new Promise((_, reject) =>
				setTimeout(
					() =>
						reject(
							new Error(`Authentication timeout after ${maxAuthTime / 1000}s`),
						),
					maxAuthTime,
				),
			),
