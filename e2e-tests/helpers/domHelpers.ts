...
export async function checkButtons(page, buttonsArray) {
	for (const buttonText of buttonsArray) {
		const buttonLocators = page.locator(
...
export async function documentReady(page, readySelector = null) {
	...
	if (readySelector) {
		try {
			...
			console.log(
				`Ready selector not visible within timeout: ${readySelector}`,
			);
		}
	}
}
...
export async function dismissOverlays(page) {
	...
	try {
		if (
			await page
				.getByRole("heading", { name: "Win with GoodParty.org Pro!" })
				.isVisible()
		) {
