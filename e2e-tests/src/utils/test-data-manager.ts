...
export class TestDataManager {
				...
				if (page.url().includes("/1")) {
					...
					if (await zipField.isVisible({ timeout: 5000 })) {
						...
						if (await levelSelect.isVisible({ timeout: 5000 })) {
							...
							console.log(
								"📍 Office search completed, selecting first office...",
							);

							// Look for the first office button/card and click it
							const officeButtons = page
								.getByRole("button")
								.filter({ hasText: /Council|Mayor|Board|Commission/ });
							...
							if (officeCount > 0) {
								...
								if (radioCount > 0) {
									...
									const anyOfficeButton = page
										.locator("button")
										.filter({ hasText: /Town|City|County|Village|Council/ });
	...
	static async cleanup(page?: Page): Promise<void> {
		console.log(
			`🧹 Cleaning up ${TestDataManager.createdUsers.length} test users...`,
		);

		if (page && TestDataManager.createdUsers.length > 0) {
			// Delete accounts via UI if page is available
			for (const user of TestDataManager.createdUsers) {
				try {
					...
					if (
						...
						try {
							...
							console.warn(
								"Login may have failed, continuing with cleanup attempt...",
							);
						}
					}
	...
	static getTrackedUsers(): TestUser[] {
		return [...TestDataManager.createdUsers];
	}
	...
	static async createAndTrackTestAccount(
		...
		try {
			...
			try {
				...
				console.log(
					`📡 Registration response: ${registrationResponse.status()}`,
				);

				if (!registrationResponse.ok()) {
					const responseText = await registrationResponse.text();
					console.error(
						`❌ Registration failed: ${registrationResponse.status()} - ${responseText}`,
					);
					throw new Error(
				...
				if (page.url().includes("/sign-up")) {
					...
					const errorMessages = await page
						.locator('[role="alert"], .error, .invalid, .text-red-500')
						.allTextContents();
					...
					const overlays = await page
						.locator('[role="dialog"], .modal, .popup, .overlay')
						.count();
					if (overlays > 0) {
						console.warn(
							`Found ${overlays} potential overlay(s) blocking form submission`,
						);
						const { NavigationHelper } = await import(
							"../helpers/navigation.helper"
						);
			...
			const dashboardContent = page.locator(
				'h1, h2, h3, main, [data-testid*="dashboard"]',
			);
			...
			if (
				...
				throw new Error(
					"Browser context was closed during account creation. This may indicate a server error or network issue.",
				);
			}
