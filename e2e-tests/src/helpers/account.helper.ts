...
export class AccountHelper {
	 ...
	 */
	static async loginWithTestAccount(
		page: Page,
	...
	static async useGlobalTestUser(page: Page): Promise<TestUser> {
		...
		if (globalUserEmail && globalUserPassword) {
			...
			await AccountHelper.loginWithTestAccount(page, globalUser);
			...
			if (page.url().includes("/onboarding/")) {
				...
				try {
					...
					console.warn(
						"⚠️ Failed to complete onboarding, tests may need to handle onboarding state",
					);
				}
			}
			...
			return await AccountHelper.createTestAccount(page);
		}
	}
