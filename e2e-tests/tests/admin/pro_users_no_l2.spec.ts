...
test('Verify admin user can access Pro users w/o voter file page', async ({page}) => {
    ...
    try {
        ...
        await page.getByRole('button', { name: 'Pro users w/o voter file' }).click();

        // Verify Pro users w/o voter file page
        await page.getByRole('heading', { name: 'Pro Users without L2 Data' }).isVisible();
        await page.getByRole('heading', { name: 'These users upgraded to pro, but we are not able to match their office with L2 data.' }).click();
