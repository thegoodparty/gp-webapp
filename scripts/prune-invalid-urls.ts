 ...
 */
const readValidationReport = async (
  reportPath: string,
 ...
 */
const extractInvalidUrls = (
  validationReport: ValidationReport,
): Set<string> => {
  const invalidUrls = new Set<string>()

  if (
    validationReport.invalidUrls &&
    Array.isArray(validationReport.invalidUrls)
  ) {
...
const parseXMLSitemap = (xmlContent: string): SitemapUrl[] => {
  ...
  urlMatches.forEach((urlBlock) => {
    ...
    if (locMatch && locValue) {
      ...
      const validChangeFreq = isValidChangeFrequency(changeFreq)
        ? changeFreq
        : 'monthly'
 ...
 */
const regenerateSitemapIndex = async (
  outputDir: string,
): Promise<IndexResult> => {
  try {
...
const pruneInvalidUrls = async (options: PruneOptions): Promise<void> => {
  ...
  try {
    ...
    console.log(
      `   Total URLs validated: ${validationReport.summary?.total || 0}`,
    )
    console.log(`   Invalid URLs found: ${invalidUrls.size}`)
    console.log(
      `   Valid URLs: ${
        (validationReport.summary?.total || 0) - invalidUrls.size
      }`,
    )
      ...
      if (dryRun) {
        try {
          ...
          const validUrls = urls.filter(
            (urlObj) => !invalidUrls.has(urlObj.url),
          )
          const removedCount = originalCount - validUrls.length
          ...
          if (removedCount > 0) {
            filesModified++
            console.log(
              `📋 ${originalCount} → ${validUrls.length} (-${removedCount})`,
            )
          } else {
        ...
        if (result.error) {
          ...
          filesModified++
          console.log(
            `✅ ${result.originalCount} → ${result.validCount} (-${result.removedCount})`,
          )
        } else {
    ...
    if (!dryRun && filesModified > 0) {
      ...
      if (indexResult.indexGenerated) {
        console.log(
          `✅ Root sitemap index (public/sitemap.xml) updated with ${indexResult.totalSitemaps} sitemaps`,
        )
      } else {
        console.log(
          `❌ Failed to regenerate root sitemap index: ${indexResult.error}`,
        )
      }
    }
    ...
    if (dryRun) {
      ...
      const reportPath = path.join(
        OUTPUT_DIR,
        `pruning-report-${new Date().toISOString().replace(/[:.]/g, '-')}.json`,
      )
      await fsPromises.writeFile(
        reportPath,
        JSON.stringify(pruningReport, null, 2),
      )
      console.log(
        `📊 Pruning report saved to: ${path.relative(
          process.cwd(),
          reportPath,
        )}`,
      )
...
if (require.main === module) {
  ...
  const reportPathArg =
    reportArgIndex !== -1 ? args[reportArgIndex + 1] : undefined
  if (reportPathArg) {
    ...
    try {
      ...
      const validationFiles = files
        .filter(
          (f) => f.startsWith('validation-report-') && f.endsWith('.json'),
        )
        .sort()
        .reverse()
  ...
  if (!validationReportPath) {
    ...
    console.error(
      'Usage: node scripts/prune-invalid-urls.js [--report <path>] [--dry-run]',
    )
    console.error(
      '   or: npm run prune-invalid-urls [-- --report <path>] [--dry-run]',
    )
    process.exit(1)
  }

  pruneInvalidUrls({ validationReportPath, dryRun }).catch((error) => {
    const errorMessage =
      error instanceof Error ? error.message : 'Unknown error'
