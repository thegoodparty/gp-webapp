...
if [ "$RANDOM_PLATFORM" == "ios" ]; then
  ...
  export ENDTEST_OPTIONS="&platform=$RANDOM_PLATFORM&os=$RANDOM_DEVICE&resolution=portrait"
...
if [ "$RANDOM_PLATFORM" == "windows" ]; then
  ...
  export ENDTEST_OPTIONS="&platform=$RANDOM_PLATFORM&os=windows10&browser=$RANDOM_BROWSER&browserVersion=latest"
  echo "ENDTEST_OPTIONS=$ENDTEST_OPTIONS" >> $GITHUB_ENV
  exit 0
fi
...
if [ "$RANDOM_PLATFORM" == "mac" ]; then
  ...
  RANDOM_BROWSER="${browser_options[$RANDOM % ${#browser_options[@]}]}"
  export ENDTEST_OPTIONS="&platform=$RANDOM_PLATFORM&os=bigsur&browser=$RANDOM_BROWSER&browserVersion=latest"
  echo "ENDTEST_OPTIONS=$ENDTEST_OPTIONS" >> $GITHUB_ENV
  exit 0
fi