...
export default function SuggestedIssues({ campaign }) {
      ...
      <div className="mt-1">
        {suggested.length > 0 &&
          suggested.map((issue, index) => (
            <div
              key={index}
              className="inline-block py-2 px-3 text-white font-medium rounded-lg bg-purple-400 mr-3 mt-3 whitespace-nowrap"
            >
              {issue}
            </div>
          ))}
