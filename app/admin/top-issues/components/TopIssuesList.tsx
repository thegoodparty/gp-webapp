...
const createPositionCallback = async (name: string, topIssueId: number) => {
  const payload = { name, topIssueId }
  const resp = await clientFetch(apiRoutes.topIssue.position.create, payload)
  return resp.data
}

const deletePositionCallback = async (id: number) => {
  const payload = { id }
  const resp = await clientFetch(apiRoutes.topIssue.position.delete, payload)
  return resp.data
}

const deleteTopIssueCallback = async (id: number) => {
  const payload = { id }
  const resp = await clientFetch(apiRoutes.topIssue.delete, payload)
  return resp.data
}

const editPositionCallback = async (id: number, name: string) => {
  const payload = { id, name }
  const resp = await clientFetch(apiRoutes.topIssue.position.update, payload)
  return resp.data
}

export const updateTopIssue = async (issue: TopIssue) => {
  const resp = await clientFetch(apiRoutes.topIssue.update, issue)
  return resp.data
}

export default function TopIssuesList(): React.JSX.Element {
  const [topIssues, setTopIssues] = useTopIssues()
  const [addNewPosition, setAddNewPosition] = useState<number | false>(false)
  const [editPosition, setEditPosition] = useState<EditPosition | false>(false)
  const [positionName, setPositionName] = useState('')
  const [showPositionDeleteAlert, setShowPositionDeleteAlert] = useState<
    number | false
  >(false)
  const [showIssueDeleteAlert, setShowIssueDeleteAlert] = useState<
    number | false
  >(false)
  ...
  const handleDeletePosition = async () => {
    if (!showPositionDeleteAlert) return
  ...
  const handleDeleteIssue = async () => {
    if (!showIssueDeleteAlert) return
    successSnackbar('Deleting...')
