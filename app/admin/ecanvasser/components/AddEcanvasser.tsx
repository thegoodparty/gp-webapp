...
interface AddEcanvasserProps {
  open: boolean
  onClose: () => void
  createCallback: () => void
}

const addEcanvasser = async (email: string, apiKey: string) => {
  try {
    const payload = {
      email,
      apiKey,
...
export default function AddEcanvasser({
  open,
  onClose,
  createCallback,
}: AddEcanvasserProps): React.JSX.Element {
