...
interface InsightHorizontalBarChartProps {
  data?: DataItem[]
  percentage?: boolean
}
...
export const InsightHorizontalBarChart = ({
  data = [],
  percentage = false,
}: InsightHorizontalBarChartProps): React.JSX.Element => {
  ...
  const renderCustomBar = (props: CustomBarProps) => {
    ...
    const path = `M ${x} ${y} 
                     ...
                     Q ${x! + width!} ${y! + height!} ${x! + width! - radius} ${
      y! + height!
    } 
                     L ${x} ${y! + height!} Z`
