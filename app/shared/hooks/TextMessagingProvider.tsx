...
export const TextMessagingProvider = ({
  ...
  const [textMessaging, setTextMessaging] =
    useState<TextMessagingData>(initialTextMessaging)

  const refreshTextMessaging = async () => {
    try {
      const resp = await clientFetch<TextMessagingData>(
        apiRoutes.textMessaging.list,
      )
      setTextMessaging(
