...
export const StepFooter = ({ numberOfSteps, currentStep, onNext, onNextText, disabledNext = false, onBack = null, onBackText = null }) => {
    return (
        <div className="flex-col items-center justify-center mb-4 mt-6">
            ...
            <div className={`w-full flex mt-6 ${onBack ? 'justify-between' : 'justify-end'}`}>
                {onBack &&
                    <Button size="large" className="" variant="ghost" onClick={onBack}>
                        {onBackText}
                    </Button>
                }
                <Button size="large" className="" variant="secondary" disabled={disabledNext} onClick={onNext}>
                    {onNextText}
                </Button>
            </div>
        </div>
