...
export const mapContactsStatsToCharts = (
  ...
  const mapAgeDistribution = (): ChartDataPoint[] => {
    ...
    buckets.forEach((bucket) => {
      ...
      else if (bucket.label === '26-35')
        consolidated['25 - 35'] += bucket.percent
      else if (bucket.label === '36-50')
        consolidated['35 - 50'] += bucket.percent
  ...
  const mapHomeowner = (): ChartDataPoint[] => {
    ...
    buckets.forEach((bucket) => {
      if (bucket.label === 'Yes') consolidated['Yes'] += bucket.percent
      else if (bucket.label === 'Likely')
        consolidated['Likely'] += bucket.percent
      else if (bucket.label === 'No') consolidated['No'] += bucket.percent
      else consolidated['Unknown'] += bucket.percent
    })
  ...
  const mapEducation = (): ChartDataPoint[] => {
    ...
    buckets.forEach((bucket) => {
      ...
      else if (bucket.label === 'High School Diploma')
        consolidated['High School Diploma'] += bucket.percent
      else if (bucket.label === 'Technical School')
        consolidated['Technical School'] += bucket.percent
      else if (bucket.label === 'Some College')
        consolidated['Some College'] += bucket.percent
      else if (bucket.label === 'College Degree')
        consolidated['College Degree'] += bucket.percent
      else if (bucket.label === 'Graduate Degree')
        consolidated['Graduate Degree'] += bucket.percent
      else consolidated['Unknown'] += bucket.percent
    })
