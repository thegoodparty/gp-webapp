...
import { Campaign, RunningAgainst } from 'helpers/types'

interface RunningAgainstSectionProps {
  campaign?: Campaign
  nextCallback?: () => void
...
export default function RunningAgainstSection({
  ...
  nextCallback,
  header,
}: RunningAgainstSectionProps): React.JSX.Element {
  ...
  const handleUpdate = (index: number | null, newValues: RunningAgainst): void => {
    ...
    setRunningAgainst((current) => {
      ...
      if (typeof index === 'number') {
        updatedRunningAgainst[index] = newValues
      } else {
        updatedRunningAgainst.push(newValues)
      }
      return updatedRunningAgainst
    })
  }
