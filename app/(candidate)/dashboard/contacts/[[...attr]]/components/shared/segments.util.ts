...
export const isCustomSegment = (customSegments: Segment[], querySegment: string): boolean => {
  return customSegments.some((segment) => segment.id === parseInt(querySegment))
}

export const isDefaultSegment = (defaultSegments: DefaultSegment[], querySegment: string): boolean => {
  return defaultSegments.some(
    (defaultSegment) => defaultSegment.value === querySegment,
  )
}

export const findCustomSegment = (customSegments: Segment[], querySegment: string): Segment | undefined => {
  return customSegments.find((segment) => segment.id === parseInt(querySegment))
}

export const filterOnlyTrueValues = (filters: Record<string, boolean>): string[] => {
