...
const ContentTutorial = ({
  newContentCallback,
  onCompleteCallback,
}: ContentTutorialProps): React.JSX.Element => {
  ...
  const onExit = () => {
    setCookie('tutorial-content', String(true))
  }
  ...
  const handleChange = (nextStepIndex: number) => {
    if (nextStepIndex === 2) {
      newContentCallback()
    }
  }
  const onBeforeChange = (nextStepIndex: number) => {
    (stepsRef.current as { updateStepElement?: (idx: number) => void })?.updateStepElement?.(nextStepIndex)
    setTimeout(() => {
      const introRef = stepsRef.current as { introJs?: { _targetElement?: Element | null } }
      const intro = introRef?.introJs
